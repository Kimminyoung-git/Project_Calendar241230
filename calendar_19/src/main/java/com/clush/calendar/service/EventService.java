...
public class EventService {
	...
	/*
	 * public Event updateEvent(Long id, Event event) { // 먼저, 데이터베이스에서 기존 이벤트 찾기
	 * Event existingEvent = eventRepository.findById(id) .orElseThrow(() -> new
	 * EntityNotFoundException("Event not found"));
	 * 
	 * // 업데이트할 필드 설정 existingEvent.setTitle(event.getTitle());
	 * existingEvent.setStart(event.getStart());
	 * existingEvent.setEnd(event.getEnd());
	 * existingEvent.setColor(event.getColor());
	 * 
	 * // 업데이트된 이벤트 저장 return eventRepository.save(existingEvent); }
	 */
    ...
    public Event updateEvent(Long id, Event event) {
        ...
        // start와 end는 이미 Date 객체로 반환되므로 추가 변환 필요 없음
        Date startDate = event.getStart();
        Date endDate = event.getEnd() != null ? event.getEnd() : startDate;  // end가 비어있으면 start로 설정
